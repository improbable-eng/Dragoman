...
const examplePolyglotConfiguration3 = polyglotConfig.Configuration.create({
    name: 'example3',
    call_config: {
        use_tls: true,
        deadline_ms: 50,
...
describe('settings UI reducer', () => {
    ...
    it('should handle ADD_POLYGLOT_CONFIG_FROM_CURRENT_FIELDS', () => {
        const settingsDataStatePolyglotConfigs = settingsData(exampleSettingsDataState, settingsDataActions.addPolyglotConfigFromCurrentFields()).polyglotConfigs;
        expect(settingsDataStatePolyglotConfigs.size).toEqual(1);
        expect(settingsDataStatePolyglotConfigs.get('example3')!).toEqual(examplePolyglotConfiguration3);
    });
});
